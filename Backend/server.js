...
// let the middleware or API routes handle the connection.

// Middleware
app.use(cors());
app.use(express.json());

// --- Static File Serving (Vercel compatibility relies on vercel.json) ---
// For local execution, we still need the public path
const publicPath = path.join(__dirname, '..', 'Public');
// No app.use(express.static) here for Vercel, but needed for local testing:
...
app.post('/api/auth/admin/login', async (req, res) => {
    ...
    if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
        const token = jwt.sign({ userId: ADMIN_USERNAME, role: 'admin' }, JWT_SECRET, { expiresIn: '1h' });
